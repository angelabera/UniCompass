...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (collegeId) {
            const college = await db_1.prisma.college.findUnique({ where: { id: collegeId } });
            if (!college) {
...
router.post('/:questionId/answers', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const answer = await db_1.prisma.answer.create({
            data: {
                content: trimmedContent,
                userId,
                questionId
